...
// 样式统一入口：照片配置 / 全局样式（字号、间距、主题色、图标模式等）
// 全部参数可选，一次调用可同时改多个
  ...
  return [
    tool(
      async (patch) => {
        ...
        const errors: string[] = [];
        const basicPatch: { photoConfig?: Record<string, unknown> } = {};
      ...
      },
      {
        ...
        description:
          "更新简历样式（全部可选，只传需要修改的项）。photoVisible：显示照片；photoBorderRadius：照片圆角 none（直角）/medium（圆角）/full（圆形）；themeColor：主题色（预设：#000000、#4b5563、#1d4ed8、#2563eb、#0d9488、#059669、#e11d48、#ea580c、#7c3aed）；fontFamily：字体（黑体/微软雅黑/宋体/楷体/仿宋）；baseFontSize：正文字号 10-24；pagePadding：页边距 0-60；paragraphSpacing：段落间距 0-40；lineHeight：行高 1-3；sectionSpacing：板块间距 0-60；headerSize：标题字号 12-36；subheaderSize：副标题字号 10-30；useIconMode：图标模式；autoOnePage：自动一页；pageBreakLinesVisible：分页线显示。",
        schema: z.object({
