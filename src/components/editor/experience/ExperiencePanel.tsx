...
export function ExperiencePanel() {
  const experiences = useResumeStore((s) => s.activeResume?.experience) || [];
  ...
  const moveExperience = (from: number, to: number) => {
    ...
    useResumeStore.getState().updateExperienceBatch(next);
  };
  ...
  return (
    <div className="flex flex-col gap-3">
      <Reorder.Group
        axis="y"
        values={experiences}
        onReorder={(items) => useResumeStore.getState().updateExperienceBatch(items as Experience[])}
...
function ExperienceItem({
  ...
  const summary = [item.company, item.position].filter(Boolean).join(" · ") || "未命名经历";
