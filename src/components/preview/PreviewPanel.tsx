...
const MAX_SCALE = 1.4; // 预览缩放上限：面板足够宽时 A4 纸最多放大到 1.4 倍
...
export function PreviewPanel() {
  ...
  // 测量预览容器宽度 → 等比缩放比例（最多 1.4 倍，收窄时缩小）。
  ...
  React.useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      if (w > 0) {
        ...
        const scale = Math.min(MAX_SCALE, Math.max(0, w - PREVIEW_SIDE_PADDING) / PREVIEW_WIDTH_PX);
  ...
  return (
    <div className="flex h-full w-full flex-col bg-[#f3f4f6]">
      ...
      <div ref={containerRef} className="preview-scroll scrollbar-hide min-h-0 flex-1 overflow-x-hidden">
